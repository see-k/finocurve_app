...
import { FileText, Upload, Download, Trash2, Shield, ChevronRight } from 'lucide-react'
...
export default function ReportsScreen() {
  ...
  const loadAll = useCallback(async () => {
    if (!window.electronAPI?.s3List) return
    setError(null)
    try {
  ...
  }, [hasElectronS3, loadAll])

  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    e.target.value = ''
    if (!file || !window.electronAPI?.s3Upload) return
  ...
  const handleDownload = async (item: FileItem) => {
    if (!window.electronAPI?.s3GetDownloadUrl) return
    try {
  ...
  const handleDelete = async (item: FileItem) => {
    if (!window.electronAPI?.s3Delete) return
  ...
  const FileRow = ({ item }: { item: FileItem }) => (
    <div className="reports-file-row">
      <div className="reports-file-row__info">
        <FileText size={20} className="reports-file-row__icon" />
    ...
    <div className="reports-page">
      ...
      <div className="reports-content">
        ...
        <GlassContainer padding="32px" borderRadius={20} className="reports-empty">
          ...
          <h2>Cloud storage in desktop app</h2>
          <p>Reports and documents are available when you run FinoCurve in the desktop app with an S3 bucket configured.</p>
        </GlassContainer>
      </div>
    </div>
  )

  if (!hasElectronS3) return emptyState
    ...
    <div className="reports-page">
      ...
      <div className="reports-content">
        ...
        <GlassContainer padding="32px" borderRadius={20} className="reports-empty">
          ...
          <p style={{ marginBottom: 20 }}>Configure your S3 bucket in Settings to store risk reports and upload documents (tax files, financial statements).</p>
          <GlassButton text="Open Cloud Storage Settings" onClick={() => navigate('/settings/cloud-storage')} isPrimary />
        </GlassContainer>
      </div>
    </div>
  )

  if (!s3Connected) return connectState
            ...
            <GlassContainer padding="16px" borderRadius={16}>
              {loading ? (
                ...
                <div className="reports-empty-list">
                  ...
                  <p>No reports yet. Generate one from Risk Analysis.</p>
                  <GlassButton text="Go to Risk Analysis" onClick={() => navigate('/risk-analysis')} width="auto" />
                </div>
              ) : (
                <div className="reports-list">
                  {reports.map((item) => <FileRow key={item.key} item={item} />)}
                </div>
              )}
            </GlassContainer>
          </div>
          ...
          <div className="reports-section">
            ...
            <GlassContainer padding="16px" borderRadius={16}>
              {loading ? (
                ...
                <div className="reports-empty-list">
                  <FileText size={32} style={{ opacity: 0.5 }} />
                  <p>No documents yet. Upload tax files, financial statements, etc. Max {MAX_FILE_SIZE_MB} MB per file.</p>
                </div>
              ) : (
                <div className="reports-list">
                  {documents.map((item) => <FileRow key={item.key} item={item} />)}
                </div>
              )}
            </GlassContainer>
          </div>
