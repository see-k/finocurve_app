...
import { buildTickerToCikMap, type SecCompanyTickerEntry } from './secTickerMap'
...
async function resolveCik(tickerOrCik: string): Promise<string | null> {
  ...
  const { data, error } = await fetchSEC<Record<string, SecCompanyTickerEntry>>(SEC_TICKERS)
